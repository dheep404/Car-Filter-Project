...
function makeUI() {
    ...
    filtersDiv.appendChild(bandButtons("price", 5))
    filtersDiv.appendChild(bandButtons("mileage", 10))
    filtersDiv.appendChild(bandButtons("year", 5))
...
function bandButtons(property, numOfBands) {
    ...
    const b = bands(property, numOfBands)  //b will be an array of band objects {from:x,to:y} - for the requested property of the cars (e.g. mileage, price)

    b.forEach(band => {
        let button = document.createElement("button")
        button.classList.add("bandBtn")
...
function bands(property, numOfBands) {
    ...
    const range = max - min
    //numberOfBands = Math.min(numberOfBands, filtered.length);

    const b = []
    for (let i = 0; i < numOfBands; i++) {
        b.push({ from: Math.floor(min + range / numOfBands * i), to: Math.floor(min + range / numOfBands * (i + 1)) })
...
function outputCars() {
    ...
    filtered.forEach(car => outputCar(car))
}
